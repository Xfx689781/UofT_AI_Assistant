...
export async function POST(req: Request) {
  try {
    ...
    const search1 = await fetch('https://api.tavily.com/search', {
      ...
      body: JSON.stringify({
        api_key: tavilyKey,
        query: courseCode + ' UofT instructor professor teaches 2024 2025 2026 timetable winter fall section',
    ...
    const search2 = await fetch('https://api.tavily.com/search', {
      ...
      body: JSON.stringify({
        api_key: tavilyKey,
        query: courseCode + ' University of Toronto professor 2023 2024 2025 2026 ratemyprofessors recent reviews',
        search_depth: 'advanced',
        max_results: 6,
      }),
    }).then(r => r.json())
    ...
    const search3 = await fetch('https://api.tavily.com/search', {
      ...
      body: JSON.stringify({
        api_key: tavilyKey,
        query: 'reddit r/UofT ' + courseCode + ' professor 2024 2025 2026 who taught recent',
        search_depth: 'advanced',
        max_results: 5,
      }),
    }).then(r => r.json())
    ...
    const systemPrompt = [
      ...
      '',
      'TEACHING STREAM vs RESEARCH STREAM:',
      'Some UofT professors are teaching stream — they focus entirely on teaching with no active research.',
      'If search data describes a professor as teaching stream, lecturer, or teaching faculty -> set hasResearch: false, researchArea: "".',
      'Do NOT fabricate research areas for teaching stream professors.',
      'Only set hasResearch: true if search data explicitly mentions publications, a lab, or active research.',
      'When in doubt, set hasResearch: false.',
      '',
