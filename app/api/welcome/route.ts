...
export async function POST(req: Request) {
  try {
    ...
    const wantsMat157 = shortLower.includes('157') || shortLower.includes('challenge') || shortLower.includes('ambitious') || shortLower.includes('push myself') || shortLower.includes('hardest') || shortLower.includes('rigorous') || shortLower.includes('analysis') || shortLower.includes('proof') || shortLower.includes('pure math') || shortLower.includes('specialist') 
    const wantsMinimalMath = shortLower.includes('minimize math') || shortLower.includes('just need to pass') || shortLower.includes('not interested in math') || shortLower.includes('hate math') || shortLower.includes('math is secondary') || shortLower.includes('easy math') || shortLower.includes('calculation') || shortLower.includes('calculus')
    ...
    const courseList = [
      ...
      'COG250Y1=Introduction to Cognitive Science (Full Year Y1)',
      'COG260H1=Data, Computation, and the Mind (H1)',
      'COG341H1=Issues in Cognitive Science I: Attention, Perception, and Consciousness (H1)',
      'COG342H1=Issues in Cognitive Science II: Concepts, Theories of Mind, and Cognitive Evolution (H1)',
      'COG343H1=Issues in Cognitive Science III: Computational Cognition (H1)',
      'COG344H1=Issues in Cognitive Science IV: Language and Communication (H1)',
      'COG345H1=Issues in Cognitive Science V: Cognitive Science and Society (H1)',
      'ACT240H1=Mathematics of Investment and Credit (H1)',
      'ACT245H1=Financial Principles for Actuarial Science I (H1)',
      'ACT247H1=Introductory Life Contingencies (H1)',
      'ACT348H1=Life Contingencies II (H1)',
      'ACT349H1=Corporate Finance for Actuarial Science (H1)',
      'ACT351H1=Loss Models: From Data to Decisions (H1)',
      'ACT370H1=Financial Principles for Actuarial Science II (H1)',
      'ACT455H1=Advanced Life Contingencies - Modeling and Applications (H1)',
      'ACT460H1=Stochastic Methods for Finance (H1)',
      'ACT470H1=Advanced Pension Mathematics (H1)',
      ...
      'PHL285H1=Aesthetics (H1)',
      'PHL301H1=Early Greek Philosophy (H1)',
      'PHL302H1=Ancient Philosophy After Aristotle (H1)',
      'PHL303H1=Plato (H1)',
      'PHL304H1=Aristotle (H1)',
      'PHL331H1=Metaphysics (H1)',
      'PHL332H1=Epistemology (H1)',
      'PHL333H1=Puzzles and Paradoxes (H1)',
      'PHL341H1=Freedom, Responsibility, and Human Action (H1)',
      'PHL342H1=Minds and Machines (H1)',
      'PHL345H1=Intermediate Logic (H1)',
      'PHL348H1=Logic and Computation (H1)',
      'PHL354H1=Philosophy of Mathematics (H1)',
    ...
    const systemPrompt = [
      ...
      '6. CSC110Y1 is SPECIAL: it is labeled Y1 but all teaching happens in Fall only. It does NOT continue into Winter. CSC111H1 is a SEPARATE H1 course in Winter — it is NOT a continuation of CSC110Y1 in the Y1 sense. Do NOT treat CSC110Y1 like other Y1 courses. Place CSC110Y1 in Fall only, CSC111H1 in Winter only, and they are independent entries.',
  ...
  '- Stats: MAT237Y1 + STA237/257H1 together if MAT137 done. Build toward STA238/261H1 in Winter.',
  ...
  '- STA237H1 needs MAT136H1/MAT137Y1 already completed — not currently taking',
...
const thirdYearTemplates = [
  ...
  'Each semester: 4-5 courses. Total: 16-20 courses across all 4 semesters.',
  '',
  ...
  '- Physics: PHY254->PHY256->PHY350->PHY354, APM426',
    ...
    const userPrompt = [
      ...
      yearType === 'third+'
        ...
        : 'Return 4 semesters. Total 12-20 courses. 3-5 per semester.',
      '',
    ...
    try {
      ...
      const finalSchedule = step3.map((sem: SemItem, idx: number) => {
        ...
        const y1s = prevSem.courses
          .filter((c: CourseItem) => {
        const code = c.code.toUpperCase().replace(/\s/g, '')
          return code.endsWith('Y1') && code !== 'CSC110Y1'
        })
