  ...
  return (
    <div className="bg-[#121922] border border-[#1e2a3a] rounded-xl p-6">
      ...
      <p className="text-sm text-[#8b9aad] mb-4">
        Select a course from your plan to see Top 2 matched professors based on your learning profile
      </p>

      {/* Course pills from plan */}
      {allowedCourses.length > 0 ? (
        <div className="flex flex-wrap gap-2 mb-4">
          {allowedCourses.map(code => (
            <button
              key={code}
              type="button"
              onClick={() => search(code)}
              className={`px-3 py-1.5 rounded-lg text-sm font-mono font-bold transition-all ${
                selectedCourse === code
                  ...
                  : 'bg-[#0a0e14] border border-[#1e2a3a] text-[#0099ff] hover:border-[#0066CC]'
              }`}
            >
              {code}
            </button>
        ...
        <p className="text-[#8b9aad] text-sm mb-4">Generate your course plan above to see professor options.</p>
      )}

      {loading && (
        <div className="space-y-3">
          <p className="text-[#8b9aad] text-sm animate-pulse">
            🔍 Searching RMP, Reddit r/UofT for {selectedCourse} professors...
          </p>
      ...
      {data && (
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-[#8b9aad]">Top {data.professors.length} matched professors</p>
          </div>

          {/* Historical note */}
          {data.isHistorical && (
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
              <p className="text-xs text-yellow-400 font-semibold mb-1">📅 Historical Reference</p>
              <p className="text-[#c8d4e0] text-xs">{data.historicalNote}</p>
            </div>
          )}
          ...
          {data.recommendedFor && (
            <div className="bg-[#002A5C]/40 border border-[#0066CC]/50 rounded-xl p-4">
              <p className="text-[#0066CC] text-xs font-semibold mb-1">🎯 TOP RECOMMENDATION FOR YOU</p>
